        ...
        promise = mongo.db('live').collection('association_sessions').count($filter)
            .then(function (n) {
                ...
                return Promise.all([
                    ...
                    // The connection should be the same per user.
            ...
            .then(function(connection) {
            	return Promise.all([
                	mongo.db('live').collection('connections').findOne({
                    	remote_connection_id: gid(connection.id)
	                })
		                .then(function(liveConnection) {
		                	return mongo.db('live').collection('users').findOne({
				                _id: liveConnection.user_id
			                })
		                }),
...
function createUser(req, res, next, connection) {
    ...
    // TODO - Check all providers and confirm the logic to extract
	// the email and handle.
    if (email) {
...
function completeConnection(req, res, next, user) {
	...
	return Promise.resolve()
		.then(function() {
			// Get the id of the the provider by remote_map_id
			return mongo.db('live').collection('providers').findOne({
            	remote_map_id: gid(req.query.map_id)
            });
        })
		.then(function(provider) {
			// Get connection by user id and provider id
        	return mongo.db('live').collection('connections').findOne({
            	user_id: gid(user._id.toString('hex')),
		        remote_connection_id: gid(req.query.existing_connection_id || req.query.connection_id),
                provider_id: provider._id
            })
		})
		.then(function(connection) {
			// If a connection exists, then update this connection and
			// delete the recently created connection
			if (connection) {
                return Promise.all([
                	Promise.resolve(connection),
                    // Get providers by remote_map_id
                    mongo.db('live').collection('providers').findOne({
                        remote_map_id: gid(req.query.map_id)
                    }),
					mongo.db('live').collection('connections').deleteOne({
						remote_connection_id: gid(req.query.connection_id)
					})
                ]);
			}
			// If connection doesn't exist, then search for the connection
			// that was recently created
			else {
				return Promise.all([
                    // Get collection that was created before connection to the third party api
                    mongo.db('live').collection('connections').findOne({
                    	remote_connection_id: gid(req.query.connection_id)
                    }),
                    // Get providers by remote_map_id
...
complete.get(function(req, res, next) {
	...
	if (req.query.existing_connection_id) {
		promises.push(validate('#/types/uuid4', req.query.connection_id));
	}
