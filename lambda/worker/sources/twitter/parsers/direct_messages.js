'use strict';

const _ = require('lodash');
const moment = require('moment');

const mongoTools = require('../../../util/mongo-tools');


module.exports = function(data, db) {
	let contacts, content, events, objectCache, tags;

	objectCache = {
		contacts: {},
		events: {},
		tags: {}
	};

	contacts = [];
	content = [];
	tags = [];
	events = new Array(data.length);

	if (data.length > 0) {
		for (let i = 0; i < data.length; i++) {
			let item = data[i];
			let message = item.message_create.message_data;

			let newTags = [];

			if (message.entities && message.entities.hashtags) {
				for (let j = 0; j < message.entities.hashtags.length; j++) {
					let tag = message.entities.hashtags[j];

					let newTag = {
						tag: tag.text,
						user_id: this.connection.user_id
					};

					if (!_.has(objectCache.tags, newTag.tag)) {
						objectCache.tags[newTag.tag] = newTag;

						tags.push(objectCache.tags[newTag.tag]);
					}

					if (newTags.indexOf(newTag.tag) === -1) {
						newTags.push(newTag.tag);
					}
				}
			}

			let newMessage = {
				identifier: this.connection._id.toString('hex') + ':::twitter:::' + item.id,
				connection_id: this.connection._id,
				provider_id: this.connection.provider_id,
				provider_name: 'twitter',
				user_id: this.connection.user_id,
				remote_id: item.id,
				'tagMasks.source': newTags,
				text: message.text,
				type: 'text'
			};

			content.push(newMessage);

			let newMediaList = [];

			if (message.attachment && message.attachment.media) {
				let mediaItem = message.attachment.media;

				let newMedia = {
					identifier: this.connection._id.toString('hex') + ':::twitter:::' + mediaItem.id_str,
					connection_id: this.connection._id,
					provider_id: this.connection.provider_id,
					provider_name: 'twitter',
					user_id: this.connection.user_id,
					url: mediaItem.expanded_url,
					remote_id: mediaItem.id_str,
					'tagMasks.source': newTags
				};

				if (mediaItem.type === 'photo') {
					newMedia.type = 'image';
					newMedia.embed_format = 'jpeg';
					newMedia.embed_content = mediaItem.media_url_https;
					newMedia.embed_thumbnail = mediaItem.media_url_https + ':thumb';
				}
				else if (mediaItem.type === 'video' || mediaItem.type === 'animated_gif') {
					let embedContent = _.find(mediaItem.video_info.variants, function(variant) {
						return variant.content_type === 'video/mp4' && variant.bitrate === 832000;
					});

					if (embedContent == null) {
						embedContent = mediaItem.video_info.variants[mediaItem.video_info.variants.length - 1];
					}

					newMedia.type = 'video';
					newMedia.embed_format = embedContent.content_type.replace('video/', '');
					newMedia.embed_thumbnail = mediaItem.media_url_https + ':thumb';
					newMedia.embed_content = embedContent.url;
				}

				content.push(newMedia);
				newMediaList.push(newMedia);
			}

			let newEvent, newContact;

			let sendingUser = item.sending_user[0];
			let receivingUser = item.receiving_user[0];

			if (this.connection.metadata.id_str === receivingUser.id_str) {
				newEvent = {
					type: 'messaged',
					context: 'Recieved direct message',
					identifier: this.connection._id.toString('hex') + ':::messaged:::twitter:::' + item.id,
					datetime: moment(new Date(item.datetime)).utc().toDate(),
					content: [newMessage].concat(newMediaList),
					connection_id: this.connection._id,
					provider_id: this.connection.provider_id,
					provider_name: 'twitter',
					user_id: this.connection.user_id
				};

				newContact = {
					identifier: this.connection._id.toString('hex') + ':::twitter:::' + sendingUser.id_str,
					connection_id: this.connection._id,
					provider_id: this.connection.provider_id,
					provider_name: 'twitter',
					user_id: this.connection.user_id,
					avatar_url: sendingUser.profile_image_url_https,
					remote_id: sendingUser.id_str,
					handle: sendingUser.screen_name,
					name: sendingUser.name
				};
			}
			else {
				newEvent = {
					type: 'messaged',
					context: 'Sent direct message',
					identifier: this.connection._id.toString('hex') + ':::messaged:::twitter:::' + item.id,
					datetime: moment(new Date(item.datetime)).utc().toDate(),
					content: [newMessage].concat(newMediaList),
					connection_id: this.connection._id,
					provider_id: this.connection.provider_id,
					provider_name: 'twitter',
					user_id: this.connection.user_id
				};

				newContact = {
					identifier: this.connection._id.toString('hex') + ':::twitter:::' + receivingUser.id_str,
					connection_id: this.connection._id,
					provider_id: this.connection.provider_id,
					provider_name: 'twitter',
					user_id: this.connection.user_id,
					avatar_url: receivingUser.profile_image_url_https,
					remote_id: receivingUser.id_str,
					handle: receivingUser.screen_name,
					name: receivingUser.name
				};
			}

			if (!_.has(objectCache.contacts, newContact.identifier)) {
				objectCache.contacts[newContact.identifier] = newContact;

				contacts.push(objectCache.contacts[newContact.identifier]);
			}

			newEvent.contacts = [objectCache.contacts[newContact.identifier]];

			events[i] = newEvent;
		}

		return mongoTools.mongoInsert({
			contacts: contacts,
			content: content,
			events: events,
			tags: tags
		}, db);
	}
	else {
		return Promise.resolve(null);
	}
};
